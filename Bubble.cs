...
public class Bubble : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (GameManager.Instance.Score % 10 == 0 && GameManager.Instance.Score != 1 && GameManager.Instance.Score != 0)
        {
            if (!GameManager.Instance.BubbleUpgraded)
            {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Bullet"))
        {
            ...
            SoundManager.Instance.PlaySound("GotHit");
            BubbleHealth--;
            HealthText.text = BubbleHealth.ToString();
        }
